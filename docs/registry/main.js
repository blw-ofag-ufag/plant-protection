...
(async () => {
  ...
  const renderComponentTile = (c) => {
    ...
    return `
      <li class="component-tile" data-uri="${c.uri}" onclick="window.open('substance.html?id=${c.uri.split('/').pop()}', '_blank', 'noopener')">
  ...
  const main = async () => {
    ...
    try {
      // Define all queries
      const sparqlProduct = `PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX : <https://agriculture.ld.admin.ch/plant-protection/> PREFIX schema: <http://schema.org/> SELECT * WHERE { GRAPH ?g { VALUES ?p { <https://agriculture.ld.admin.ch/plant-protection/${id}> } ?producttype rdfs:subClassOf+ :Product . ?p a ?producttype ; schema:name ?productName ; :federalAdmissionNumber ?federalNo ; :hasPermissionHolder ?company . OPTIONAL { ?p :foreignAdmissionNumber ?foreignNo . } OPTIONAL { ?p :formulation ?fc . ?fc schema:name ?formLabel . FILTER (lang(?formLabel) = "de") } OPTIONAL { ?producttype schema:name ?producttypeLabel . FILTER (lang(?producttypeLabel) = "de") } } OPTIONAL { ?p :hasCountryOfOrigin ?c . ?c schema:name ?countryName . FILTER (lang(?countryName) = "de") } OPTIONAL { ?p :isSameProductAs ?sameProduct . ?sameProduct schema:name ?sameProductName . OPTIONAL { ?sameProduct :hasCountryOfOrigin/schema:alternateName ?sameCountryCode . } } }`;
      const sparqlComponents = `PREFIX : <https://agriculture.ld.admin.ch/plant-protection/> PREFIX schema: <http://schema.org/> PREFIX obochebi: <http://purl.obolibrary.org/obo/chebi/> SELECT * WHERE { GRAPH ?g { :${id} :hasComponentPortion ?portion . ?portion :substance ?substance ; :role ?roleIRI . ?substance schema:name ?subName . ?roleIRI schema:name ?roleName . FILTER(lang(?subName)="de"||lang(?subName)="") FILTER(lang(?roleName)="de") OPTIONAL{?portion :hasGrammPerLitre ?grams} OPTIONAL{?portion :hasPercentage ?pct} OPTIONAL{ ?substance :hasChebiIdentity ?chebiIRI} OPTIONAL{ ?substance obochebi:formula ?formula} OPTIONAL{ ?substance obochebi:smiles ?smiles} } } ORDER BY DESC(?pct) DESC(?grams)`;
      const sparqlIndications = `PREFIX : <https://agriculture.ld.admin.ch/plant-protection/> PREFIX schema: <http://schema.org/> SELECT * WHERE { GRAPH ?g { :${id} :indication ?ind . ?ind :applicationArea ?areaUri . ?areaUri schema:name ?area . FILTER(lang(?area)="de") ?ind :cropGroup ?cropUri . ?cropUri schema:name ?cropLabel . FILTER(lang(?cropLabel)="de") ?ind :cropStressor ?pestUri . ?pestUri schema:name ?pestLabel . FILTER(lang(?pestLabel)="de") OPTIONAL { ?ind :notice ?obl . ?obl schema:name ?oblLabel . FILTER(lang(?oblLabel)="de") } OPTIONAL { ?ind :dosage ?d . OPTIONAL { ?d schema:minValue ?dosageMin . } OPTIONAL { ?d schema:maxValue ?dosageMax . } OPTIONAL { ?d schema:unitCode/schema:name ?dosageUnit . FILTER(lang(?dosageUnit)="de") } } OPTIONAL { ?ind :expenditure ?e . OPTIONAL { ?e schema:minValue ?expMin . } OPTIONAL { ?e schema:maxValue ?expMax . } OPTIONAL { ?e schema:unitCode/schema:name ?expUnit . FILTER(lang(?expUnit)="de") } } OPTIONAL { ?ind :waitingPeriod ?w . OPTIONAL { ?w schema:value ?waitValue . } OPTIONAL { ?w schema:unitCode/schema:name ?waitUnit . FILTER(lang(?waitUnit)="de") } } } }`;
      const sparqlHazards = `PREFIX :<https://agriculture.ld.admin.ch/plant-protection/> PREFIX schema:<http://schema.org/> SELECT * WHERE { GRAPH ?g { :${id} :notice ?statement . ?statement schema:name ?label ; a/schema:name ?class . FILTER(lang(?label)="de") OPTIONAL{ ?statement :hasHazardStatementCode ?code } VALUES ?class { "R-Satz"@de "S-Satz"@de "Gefahrensymbol"@de "Signalwort"@de } } }`;
