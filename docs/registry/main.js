...
(async () => {
  ...
  const main = async () => {
    ...
    try {
      // Define all queries
      const sparqlProduct = `PREFIX : <https://agriculture.ld.admin.ch/plant-protection/> PREFIX schema: <http://schema.org/> SELECT * WHERE { GRAPH ?g { VALUES ?p { <https://agriculture.ld.admin.ch/plant-protection/${id}> } ?p a ?producttype ; schema:name ?productName ; :federalAdmissionNumber ?federalNo ; :hasPermissionHolder ?company . OPTIONAL { ?p :foreignAdmissionNumber ?foreignNo . } OPTIONAL { ?p :formulation ?fc . ?fc schema:name ?formLabel . FILTER (lang(?formLabel) = "de") } OPTIONAL { ?producttype schema:name ?producttypeLabel . FILTER (lang(?producttypeLabel) = "de") } } OPTIONAL { ?p :hasCountryOfOrigin ?c . ?c schema:name ?countryName . FILTER (lang(?countryName) = "de") } OPTIONAL { ?p :isSameProductAs ?sameProduct . ?sameProduct schema:name ?sameProductName . OPTIONAL { ?sameProduct :hasCountryOfOrigin/schema:alternateName ?sameCountryCode . } } }`;
